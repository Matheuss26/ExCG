...
float shoulder = 0.0f, elbow = 0.0f, segmento3 = 0.0f, garra=0.0f,base = 0.0f, pontaD =0.0f,pontaE=0.0f;
int selected = 1;
...
void desenhaBraco () {
   ...
   glRotatef (segmento3, 0.0, 0.0, 1.0);
   glTranslatef (1.0, 0.0, 0.0);
   glPushMatrix();
      glScalef (2.0, 0.5, 0.5);
      glutSolidCube (1.0);
...
void specialKeys(int key, int x, int y)
{
   switch(key)
   {
      case GLUT_KEY_LEFT:
            ...
            if(selected == 3) segmento3 = ((int) segmento3 + 5) % 360;
            if(selected == 4){
